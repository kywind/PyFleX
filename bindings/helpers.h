						...
						Vec3 skinOffset=0.0f, float skinExpand=0.0f, Vec4 color=Vec4(0.0f), float springStiffness=0.0f)
{
	...
	if (skin)
	{
		...
		// std::cout << "g_mesh->GetNumVertices() = " << g_mesh->GetNumVertices() << std::endl;
		for (uint32_t i=startVertex; i < g_mesh->GetNumVertices(); ++i)
		{
			...
			if (g_mesh->m_colours.size() < g_mesh->GetNumVertices())
			{
				g_mesh->m_colours.resize(g_mesh->GetNumVertices());
			}
			if (LengthSq(color) == 0.0f)
				g_mesh->m_colours[i] = mesh.m_colours[i-startVertex];
			else
...
NvFlexDistanceFieldId CreateSDF(const char* meshFile, int dim, float margin = 0.1f, float expand = 0.0f, bool texture=false)
{
	Mesh* mesh = ImportMesh(meshFile, texture);
...
// written by Yunzhu
void CreateShirt(Vec3 lower, int dx, int dy, int dz, float radius, int phase, float stretchStiffness, float bendStiffness, float shearStiffness, Vec3 velocity, float invMass) {
    int tx = dx + dz * 4;
    int ty = dy;
    ...
    for (int y = 0; y < dy; ++y) {
        for (int x = dz * 2; x < dz * 2 + dx; ++ x) {
			...
			g_buffers->velocities.push_back(velocity);
			g_buffers->phases.push_back(phase);
			g_buffers->uvs.push_back(Vec3(float(x)/tx, float(y)/ty, 0.0f));

            mask[x][y] = int(g_buffers->positions.size()) - 1;
        }
    }

    // left sleeve
    for (int y = 0; y < dz * 2; ++y) {
        for (int x = dz * 2 - 1 - y; x < min(dz * 2 - 1 - y + dz, dz * 2); ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
    ...
    for (int y = dz * 2; y < dz * 2 + (dz - 1) / 2; ++y) {
        for (int x = y - dz * 2 + 1; x < dz * 3 - y - 1; ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
    ...
    // right sleeve
    for (int y = 0; y < dz * 2; ++y) {
        for (int x = max(dz * 2 + dx + y - dz + 1, dz * 2 + dx); x < dz * 2 + dx + y + 1; ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
    ...
    for (int y = dz * 2; y < dz * 2 + (dz - 1) / 2; ++y) {
        for (int x = y + dx + dz * 1 + 1; x < dx + dz * 6 - y - 1; ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
    ...
    */
    ...
    for (int y = 1; y < ty; ++y) {
        for (int x = 1; x < tx; ++x) {
            if (mask[x - 1][y - 1] >= 0 && mask[x][y - 1] >= 0 && mask[x][y] >= 0) {
                g_buffers->triangles.push_back(mask[x - 1][y - 1]);
			    g_buffers->triangles.push_back(mask[x][y - 1]);
                g_buffers->triangles.push_back(mask[x][y]);
			    g_buffers->triangleNormals.push_back(Vec3(0.0f, 1.0f, 0.0f));
            }
            if (mask[x - 1][y - 1] >= 0 && mask[x][y] >= 0 && mask[x - 1][y] >= 0) {
                g_buffers->triangles.push_back(mask[x - 1][y - 1]);
                g_buffers->triangles.push_back(mask[x][y]);
                g_buffers->triangles.push_back(mask[x - 1][y]);
			    g_buffers->triangleNormals.push_back(Vec3(0.0f, 1.0f, 0.0f));
            }
        }
    }
    ...
    for (int y = 0; y < ty; ++y) {
        for (int x = 0; x < tx; ++x) {
            if (x < tx - 1 && y < ty - 1) {
                if (mask[x][y] >= 0 && mask[x + 1][y] >= 0 && mask[x][y + 1] >= 0 && mask[x + 1][y + 1]== -1) {
                    g_buffers->triangles.push_back(mask[x][y]);
			        g_buffers->triangles.push_back(mask[x + 1][y]);
                    g_buffers->triangles.push_back(mask[x][y + 1]);
			        g_buffers->triangleNormals.push_back(Vec3(0.0f, 1.0f, 0.0f));
		...
		for (int x = 0; x < tx; ++x) {
            if (mask[x][y] >= 0) {
			    ...
			    if (x > 0 and mask[x - 1][y] >= 0) {
				    int index1 = mask[x - 1][y];
				    CreateSpring(index0, index1, stretchStiffness);
			    }

			    if (x > 1 && mask[x - 2][y] >= 0) {
				    int index2 = mask[x - 2][y];
				    CreateSpring(index0, index2, bendStiffness);
			    }

			    if (y > 0 && x < tx - 1 && mask[x + 1][y - 1] >= 0) {
				    int indexDiag = mask[x + 1][y - 1];
				    CreateSpring(index0, indexDiag, shearStiffness);
			    }

			    if (y > 0 && x > 0 && mask[x - 1][y - 1] >= 0) {
				    int indexDiag = mask[x - 1][y - 1];
				    CreateSpring(index0, indexDiag, shearStiffness);
			    }
		    }
    ...
    for (int x = 0; x < tx; ++x) {
		for (int y = 0; y < ty; ++y) {
            if (mask[x][y] >= 0) {
			    ...
			    if (y > 0 && mask[x][y - 1] >= 0) {
				    int index1 = mask[x][y - 1];
				    CreateSpring(index0, index1, stretchStiffness);
			    }

			    if (y > 1 && mask[x][y - 2] >= 0) {
				    int index2 = mask[x][y - 2];
				    CreateSpring(index0, index2, bendStiffness);
			    }
            }
		}
...
// written by Yunzhu
void CreatePants(Vec3 lower, int dx, int dy, int dz, float radius, int phase, float stretchStiffness, float bendStiffness, float shearStiffness, Vec3 velocity, float invMass) {
    int tx = dx;
    int ty = dy + dz;
    ...
    for (int y = dy; y < dy + 2; ++y) {
        for (int x = (dx - 4) / 2; x < (dx - 4) / 2 + (dy + 2 - y); ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
    ...
    for (int y = dy; y < dy + 2; ++y) {
        for (int x = (dx - 4) / 2 + 2 + (y - dy) ; x < (dx - 4) / 2 + 4; ++x) {
            float z = 0;
            Vec3 position = lower + radius * Vec3(float(x), float(z), float(y));
	...
	for (int z=0; z < dz; ++z)
	{
		for (int y=0; y < dy; ++y)
		{
			for (int x=0; x < dx; ++x)
			{
				...
				g_buffers->velocities.push_back(velocity);
				g_buffers->phases.push_back(phase);
				g_buffers->uvs.push_back(Vec3(float(x)/dx, float(y)/dy, 0.0f));
...
Mesh* CreateRandomConvexMesh(int numPlanes, float minDist, float maxDist) {
	...
	int m = NvFlexCreateConvexMesh(g_flexLib);
	...
	for (uint32_t j = 0; j < builder.mIndices.size(); j += 3)
	{
		...
		int startIndex = mesh->m_positions.size();

		mesh->m_positions.push_back(Point3(builder.mVertices[a]));
		mesh->m_normals.push_back(n);

		mesh->m_positions.push_back(Point3(builder.mVertices[b]));
		mesh->m_normals.push_back(n);

		mesh->m_positions.push_back(Point3(builder.mVertices[c]));
		mesh->m_normals.push_back(n);
